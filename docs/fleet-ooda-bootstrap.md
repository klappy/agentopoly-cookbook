...
- **G2 repo write — path depends on the surface, and this gate names which one it used:**
  - *Chat / Claude Code:* Git Auth `github_token` with `{"contents":"write","pull_requests":"write"}`, `repositories:["agentopoly-cookbook"]`. Clone `https://x-access-token:<token>@github.com/klappy/agentopoly-cookbook.git`. Prove write by opening and squash-merging a one-line PR that appends `bootstrap G2 ok <timestamp>` to `journal/<today>-claude-fleet-ooda.md` (create the file from `journal/TEMPLATE.md` if absent). **Never use the sandbox git proxy** — `access denied by the git proxy … not in this session's authorized repository set` means you skipped this gate.
  - *Cowork / scheduled:* **Never use the sandbox git proxy.** Git Auth tokens cannot be exercised from this sandbox (receipt `journal/inbox/2026-09-13T133738Z-bootstrap-receipt.md`); the proxy 403s this repo unless it is already in the session's authorized set, and a scheduled task cannot observe or attach that. **G2 is not a halt.** Write the journal and the receipt to the project (`claude/journal/`, `claude/wake-receipts/`) and continue to G3 — play never needed the repo. The next chat boarding lands those files on `main` per AGENTS.md step 0 (append missing journal sections; never overwrite an existing `journal/` file).
...
0. **Receipt file first:** create `claude/wake-receipts/<UTC-timestamp>-<SEAT>.md` in this project; append a line per step below as it happens. Then pass G1 (canon), G2 (repo write — Git Auth in chat/Code, **never the sandbox git proxy**; in Cowork/scheduled write the journal and receipt to the project and keep flying — a failed or unattempted push is not a halt), G3 (this seat resolves to its name — Klappy via the Agentopoly connector, Homesteads via raw MCP with the project token file, never echoed). A failed gate is written to the receipt file and reported; play continues only for gates that are not needed for play (G2 is not needed to play — fly the seat, save the journal to the project, report the merge failure verbatim).
