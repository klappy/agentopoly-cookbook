...
## Captain ruling — 2026-09-08 ~00:2xZ
[D] **Option 3: keep HomestarRunner-2 as a second Homestead (Ethiopia cluster).** Reasoning accepted by the captain: the operator has punished *account type* (landless drones, loop rent), not account count — Otto, CoS, EA, Ceviche, HomestarRunner are all untouched. HR-2 holds a real set, a real hub, and rent from real arrivals; it is a Homestead in everything but the accident of its name. Fly it under `docs/homestead.md` exactly like Kenya: zero drones, never sell land, tithe from surplus only, journal every session. Any operator notice about it is itself the halt and the v1 result.
[H] HR-2 handoff: Ethiopia ✅ L2 + Addis hub · Mogadishu single (hold, harmless) · cash M$398 · L3 when cash ≥ M$470 · then second proven ≥2-city set within cab reach. Its token lives in the Cowork session that minted it; that session (or the captain's seat file) owns the next run. Registered active in `fleet/README.md`.
