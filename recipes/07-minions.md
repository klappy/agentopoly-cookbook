...
> One runner loops many disposable drone MCP seats. Not crowns. Not namesakes. Zero holdings. Travel only **team-engine L3 tiles**. Never buy, build, buyout, complete sets, or write tokens. Pack of 3 behind one runner; mint/scale only after captain Serve. Attention + ToS are the ceiling.
...
- Scale: only on captain Serve (double-to-6 is a later Serve, not this file).
...
## Done
Recipe on disk. PR open. **Mint happens only after captain Serve** — pack of 3, then stop. This file is not a Serve. This recut does not mint and does not change the namesake roster.
