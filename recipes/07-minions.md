...
> One runner loops many disposable drone MCP seats. Not crowns. Not namesakes. Zero holdings. Travel only **team-engine L3 tiles**. Never buy, build, buyout, complete sets, or write tokens. Pack of 12 live behind one runner; this wave Otto+Auggie (no NZ); cash floor on; target 20/engine still later Serve. Attention + ToS are the ceiling.
...
4. **Loop engines** (verify live L3 before landing): **Otto Denmark** · **Auggie Bolivia** · **CoS New Zealand**. **This wave:** Otto + Auggie only (**no NZ**). **Chile:** skip while L1 (or only if a live rent check shows it pays) — not a primary loop stop. **Forbidden as Minion targets:** flagship Japan / Tokyo / Mexico / Uruguay / Canada, and any other flagship-corridor tile.
...
2. Pick the next **team-engine** stop. **This wave:** Otto Denmark / Auggie Bolivia (**no NZ**). Never Japan. Never flagship. Skip Chile while L1.
...
- **This wave:** bias **Otto + Auggie** (**no NZ**). Cash floor on.
...
## Done
Recipe on disk. Pack of **12 live** (Otto reported). This wave Otto+Auggie (**no NZ**). Cash floor on. Target **20/engine** still later Serve. This file is not a mint and does not change the namesake roster.
