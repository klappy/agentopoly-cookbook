...
# 🛫 Fleet OODA — bootstrap

## Cadence — two touches a day, every seat, including the flagship (captain ruling 2026-09-16 00:2xZ: "Consistency is the only way we'll win. Keep all homesteads running including flagship.")
- **Wake A: 00:05–06:00Z.** Hash → notifications → salary → status → one own-tile move per seat (the flagship flies its venture lap: Tokyo → Mexico City → Johannesburg → Cape Town → Montevideo) → take every contract the move drops.
- **Wake B: 12:05–18:00Z.** Same loop; the **second-shift contract unlocks 12:00Z** and salary caps ~11:45–12:01Z. This is the wake that makes a **perfect day (+M$150 per seat)**.
- Tills go stale (raidable) 6h after the last collect: a lap every ≤12h keeps them safe.
- A wake may fly without writing back (09-14 ruling); the chat sweep records it. Missing a wake costs ~M$1,200 salary per seat, the streak, and any stale till — the gap is always the most expensive rival.
: test the whole loop once, then schedule it per seat
...
1. In the session's *Add context* sheet: **Connectors** → enable **oddkit**, **Agentopoly**, **Git Auth**. **Permission → switch off Auto.** The thing that blocked the Git Auth token in Cowork was the *auto-mode* classifier (`denied by the Claude Code auto mode classifier` — receipt in `journal/inbox/`); with manual permission the session asks you instead of denying. There is no "add repo to sources" control in the mobile app — that phrase came from a proxy error string, not from anything you can click.
...
- **G2 repo write — path depends on the surface, and this gate names which one it used:**
  ...
  - *Cowork:* under **Permission = Auto**, the auto-mode classifier denies every use of a Git Auth token (file, inline URL, REST, relay — receipt `journal/inbox/2026-09-13T133738Z-bootstrap-receipt.md`), and the sandbox git proxy only injects credentials for repos it already knows about, which a mobile-launched session has no control for. **First try: Permission off Auto, then the Git Auth path exactly as in chat** — the captain approves the push when asked. If the push fails: **G2 is not a halt.** Write the journal and the receipt to the project (`claude/journal/`, `claude/wake-receipts/`) and continue to G3 — play never needed the repo. The next chat boarding sweeps `claude/journal/*` and `claude/wake-receipts/*` from the project into `journal/` and `journal/inbox/` on `main` (that sweep is step 0 of every chat session from now on).
