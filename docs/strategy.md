...
- **Contracts never override doctrine (2026-09-10).** Daily contracts and cards are chased only where they coincide with §2.3 / `docs/homestead.md` (own-set spend, arrivals, new sets, new countries). A contract that asks for a doctrine violation — *Buy out a rival's city*, anything requiring a landing on a fleet or rival L3 tile while loaded — is left to expire. The flagship does not travel for contracts: at 36 building levels the expected card cost exceeds any contract's pay.
- **One seat, one clock (captain ruling 2026-09-10).** Fleet seats never play as a synchronised burst on a quiet board. Each runs its own hourly OODA wake on a fixed staggered offset with 1–7 minutes of random delay before the first call, at most one claim-or-build objective per wake, and skips the wake when nothing in doctrine is fundable. Schedule and per-seat offsets: `fleet/DIRECTIVE-2026-09-10.md`.
